...
class ContentOverlay extends StatelessWidget {
  final String title;
  final bool isLocked;
  final List<String> collaboratorNames; // Using real names from Firebase
  ...
  Widget _buildAvatar(int index, String name) {
    const List<Color> avatarColors = [Color(0xFFE57373), Color(0xFF64B5F6), Color(0xFFFFB74D)];
    final Color borderColor = isLocked ? Colors.white : Colors.black87;
    ...
    return Container(
      ...
      child: CircleAvatar(
        ...
        child: Text(initial, style: const TextStyle(fontSize: 10, color: Colors.white, fontWeight: FontWeight.bold)),
      ),
    );
  }
  ...
  Widget _buildAvatarStack() {
    ...
    const double overlapPixels = 8;
    const int maxVisible = 3;
    final int visibleCount = collaboratorNames.length > maxVisible ? maxVisible : collaboratorNames.length;
    final int remaining = collaboratorNames.length - maxVisible;

    final Color counterBg = isLocked ? kInactiveColor : Colors.black;
    final Color counterBorder = isLocked ? Colors.white : Colors.black;

    List<Widget> layers = [];
    for (int i = 0; i < visibleCount; i++) {
      layers.add(Positioned(
        left: i * (2 * avatarRadius - overlapPixels),
        child: _buildAvatar(i, collaboratorNames[i]),
      ));
    }

    if (remaining > 0) {
      layers.add(Positioned(
        left: visibleCount * (2 * avatarRadius - overlapPixels),
        child: Container(
          decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: counterBorder, width: 2), color: counterBg),
          child: CircleAvatar(
            radius: 10,
    ...
    double stackWidth = (visibleCount * 2 * avatarRadius) - ((visibleCount - 1) * overlapPixels) + (remaining > 0 ? (2 * avatarRadius - overlapPixels) : 0);

    return SizedBox(width: stackWidth, height: 26, child: Stack(children: layers));
  }
        ...
        children: [
          Expanded(
            child: Center(
              child: Text(
                title,
                style: TextStyle(color: foregroundColor.withOpacity(0.9), fontWeight: FontWeight.w900, fontSize: 20),
                textAlign: TextAlign.center,
                maxLines: 2,
                overflow: TextOverflow.ellipsis,
              ),
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.end,
            children: [
              Text('Members:', style: TextStyle(color: foregroundColor, fontSize: 13, fontWeight: FontWeight.w500)),
